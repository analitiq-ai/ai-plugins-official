...
### Changed
- Restructured agent chain: wizard → registry-browser → connection-creator → private-endpoint-creator → pipeline-builder → stream-builder × N
...
- Wizard now interviews for endpoint selection after connections are created

### Removed
- Endpoint data mapper agent (replaced by stream-builder)
- Pipeline assembler agent (replaced by pipeline-builder + wizard assembly)
